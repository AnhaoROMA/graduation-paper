...
\par 对高光谱图像的降噪主要是利用了图像本身的低秩特性。由于基于秩最小化的降噪方法是一个NP-hard问题，最初，研究人员使用秩函数的凸近似，也就是核范数，去替代秩函数。尽管基于核范数最小化的降噪方法已经被应用于很多方面，它还是存在忽略奇异值的实际含义等方面的缺点。近几年，一些对秩函数的非凸的放缩方式被提出，并被应用于图像降噪、矩阵补全等实际问题。对这方面的研究路线可被概括成：提出对秩函数的放缩方式、建立最优化模型、求解模型。
...
\par 高光谱图像将地物的空间分布信息与光谱特征信息结合在一起，即：在二维空间分布信息的基础上添加了一维光谱特征信息。因此，高光谱图像可以被看成是一个立方体形式的数据：每个波段都是一幅二维的平面图像，而每个像素点都是一条光谱特征曲线。由于不同物质在相同波段内的辐射强度不同，不同物质在高光谱图像中会表现出不同的光谱特征曲线\cite{howtogenanhsi}，如图\ref{img-pricipleofphotograph}所示。通过对地物的连续成像，高光谱图像具有多而窄的波段，因此包含了丰富的光谱特征信息。
...
\newpage
\section{实验结果与分析}\label{analysis}
\subsection{实验结果}
\subsubsection{人造数据部分}
...
\bibitem{usageofhsi-2}Using hyperspectral imagery to estimate forest floor consumption from wildfire in boreal forests of Alaska, USA.
\bibitem{usageofhsi-3}An assessment of independent component analysis for detection of military targets from hyperspectral images.
\bibitem{further-use-1}Weighted Generalized Nearest Neighbor for Hyperspectral Image Classification.
\bibitem{further-use-2}Two-stage Clustering Technique Based on the Neighboring Union Histogram for Hyperspectral Remote Sensing Images.
\bibitem{further-use-3}A spectral-spatial method based on low-rank and sparse matrix decomposition for hyperspectral anomaly detection.
\bibitem{TNN-1}Matrix completion by truncated nuclear norm regularization.
\bibitem{log-norm}Matrix Completion Based on Non-Convex Low-Rank Approximation.

\bibitem{Review}Low Rank Regularization: A review.
\bibitem{WNN-1}Weighted Nuclear Norm Minimization with Application to Image Denoising.
\bibitem{CNN-1}Robust principal component analysis via capped norms.
\bibitem{CNN-2}Joint capped norms minimization for robust matrix recovery.
...
\bibitem{Tensor-1}Tensor robust principal component analysis: Exact recovery of corrupted low-rank tensors via convex optimization.
