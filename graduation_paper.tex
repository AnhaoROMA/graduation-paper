...
\par 对高光谱图像的降噪主要是利用了图像本身的低秩特性。由于基于秩最小化的降噪方法是一个NP-hard问题，最初，研究人员使用秩函数的凸近似，也就是核范数，去替代秩函数。尽管基于核范数最小化的降噪方法已经被应用于很多方面，它还是存在忽略奇异值的实际含义等方面的缺点的。近几年，一些对秩函数的非凸的放缩方式被提出，并被应用于图像降噪、矩阵补全等实际问题。对这方面的研究路线可被概括成：提出对秩函数的放缩方式、建立最优化模型、求解模型。
...
\label{Rank-equation}
\end{equation}
\par 但是，在对高光谱图像进行降噪时，既需要去除图像里的噪声，同时也需要尽可能地保留原来的信息\cite{log-norm}，故拥有最小秩的矩阵并非实际应用中需要的\cite{log-norm}。因此，实际上，需要求解的最优化问题并非是式\ref{Rank-equation}，而是：
\begin{equation}
\centering
...
\begin{figure}[h]
    ...
    \label{svd-example}
\end{figure}
\par 式\ref{Rank-equation}（或式\ref{F-rank}）是基于秩最小化（Rank Minimization, RM）的降噪方法，然而，这是一个NP-hard问题\cite{TNN-1}。况且，秩函数（$rank()$）是不连续的\cite{TNN-1}。所以，通常使用秩函数的凸近似，即核范数，作为秩函数的替代\cite{TNN-1}，即：
\begin{equation}
\centering
...
\subsection{使用$log$-核范数替代核范数的降噪方法}
...
\subsection{实验设计}
\par 与一般的RGB图像只含有R、G、B三个通道不同，高光谱图像的通道数量众多。以Urban数据集\footnote{数据来源：\url{http://www.tec.army.mil/hypercube}}为例，Urban数据集内的图像规格为：$304 \times 304 \times 210$，即图像包含210个通道。因此，高光谱图像通常只能目视其中某一通道，而无法同时直接目视全部通道。基于这点限制，本文的实验分为两个部分。
...
\par 二是对真实的、自然获取到的高光谱图像进行降噪。这部分的实验用图像均是在生产实践中获取的，在其他研究人员的论文中被称为真实视觉数据（Real Visual Data），由于无法获取它们的无噪声的原图像，加之无法同时显示所有通道，故仅能展示其中某些通道经过不同降噪方法后得到的去噪图像，用于目视效果的对比。由于高光谱图像通常不是8bit图，故在对原图进行降噪后，还需要进行像素值的映射，将高位图转为一般的8位图。
\subsection{实验数据}
...
\begin{figure}[h]
    ...
    \subfigure[高光谱图像的第4通道]{
        \includegraphics[scale=0.6]{band-4.jpg}
        \label{band-4}
    }
    \subfigure[高光谱图像的第31通道]{
        \includegraphics[scale=0.6]{band-31.jpg}
        \label{band-31}
    }
    
    \subfigure[高光谱图像的第98通道]{
        \includegraphics[scale=0.6]{band-98.jpg}
        \label{band-98}
    }
    \subfigure[高光谱图像的第198通道]{
        \includegraphics[scale=0.6]{band-198.jpg}
        \label{band-198}
    }
