...
\subsection{实验设计}
\par 与一般的RGB图像只含有R、G、B三个通道不同，高光谱图像的通道数量众多。以Urban数据集\footnote{数据来源：\url{http://www.tec.army.mil/hypercube}}为例，Urban数据集内的图像规格为：$304 \times 304 \times 210$，即图像包含210个通道。因此，高光谱图像通常只能目视其中某一通道，而无法直接目视全部通道。基于这点限制，本文的实验分为两个部分。
...
\par 二是对真实的、自然获取到的高光谱图像进行降噪。这部分的实验用图像均是在生产实践中获取的，在其他研究人员的论文中被称为真实视觉数据（Real Visual Data），由于无法获取它们的无噪声的原图像，加之无法同时显示所有通道，故仅能展示其中某些通道经过不同降噪方法后得到的去噪图像，用于目视效果的对比。
\subsection{实验数据}
...
\item 实验1
\par 含方差为10的零均值高斯噪声的图像

\item 实验2
\par 含方差为30的零均值高斯噪声的图像

\item 实验3
\par 含方差为50的零均值高斯噪声的图像

\item 实验4
\par 含方差为70的零均值高斯噪声的图像
...
\item 实验9
\par 含10\%椒盐噪声与方差为10的零均值高斯噪声的图像
\item 实验10
\par 含10\%椒盐噪声与方差为30的零均值高斯噪声的图像
\item 实验11
\par 含20\%椒盐噪声与方差为30的零均值高斯噪声的图像
\end{itemize}
